...
class Engine(object):
    ...
    def _logger(self, model):
        ...
        self.args.checkpoint = 'checkpoint{}_{}_{}_timestamp[{}]'.format(
            str(self.args.seed),
            str(self.args.data.val_dataset_list),
            str(self.model.get_setting(self.args)),
            timestamp
        )
        ...
        logger = get_logger(
            ...
            __name__,
            'record_s' + str(self.args.seed) + '.log'
        )

        logger.info(self.args)

        logger.info(
            "Ablation flags | use_pmrl_svd: {}, lambda_svd: {}, use_vot_dpe: {}, "
            "use_text_dpe: {}, use_image_dpe: {}, use_trend: {}, use_seasonal: {}, "
            "dpe_trend_queries: {}, dpe_seasonal_queries: {}, dpe_num_heads: {}".format(
                getattr(self.args, "use_pmrl_svd", False),
                getattr(self.args, "lambda_svd", 0.0),
                getattr(self.args, "use_vot_dpe", False),
                getattr(self.args, "use_text_dpe", True),
                getattr(self.args, "use_image_dpe", True),
                getattr(self.args, "use_trend", True),
                getattr(self.args, "use_seasonal", True),
                getattr(self.args, "dpe_trend_queries", None),
                getattr(self.args, "dpe_seasonal_queries", None),
                getattr(self.args, "dpe_num_heads", None),
            )
        )
