...
class Engine_Forecasting(object):
    ...
    def _unpack_model_output(self, model_out):
        if len(model_out) == 5:
            y_hat, multimodal, multimodal1, multimodal2, balance_loss = model_out
            svd_loss = y_hat.new_tensor(0.0)
            confu_loss = y_hat.new_tensor(0.0)
            dpe_aux = {}

        elif len(model_out) == 7:
            y_hat, multimodal, multimodal1, multimodal2, balance_loss, svd_loss, dpe_aux = model_out
            confu_loss = y_hat.new_tensor(0.0)

        elif len(model_out) == 8:
            y_hat, multimodal, multimodal1, multimodal2, balance_loss, svd_loss, confu_loss, dpe_aux = model_out

        else:
            raise ValueError(f"Unexpected model output length: {len(model_out)}")

        return y_hat, multimodal, multimodal1, multimodal2, balance_loss, svd_loss, confu_loss, dpe_aux

    def train_batch(self, batch, model, optimizer, batch_num):
        model.train()
        optimizer.zero_grad()

        x, start_days, start_intervals, poi, satellite, dreamkg, loc, adj, dreamkg_adj, y, mean, std, state, dataset, topk_indices, topk_values = [
        ...
        with torch.cuda.amp.autocast(dtype=torch.bfloat16):
            model_out = model(
                x, start_days, start_intervals, poi, satellite, dreamkg, loc,
                adj, dreamkg_adj, mean, std, state, topk_indices, topk_values
            )

            y_hat, multimodal, multimodal1, multimodal2, balance_loss, svd_loss, confu_loss, dpe_aux = self._unpack_model_output(model_out)

            if self.args.is_norm:
                mean_std = np.load(
                    os.path.join(self.args.data.root_path, "data", "mean_std.npy"),
                    allow_pickle=True
                ).item()
            ...
            if multimodal1 is not None:
                ...
                con_loss = y_hat.new_tensor(0.0)

            if balance_loss is not None:
                balance_loss = 10 * balance_loss
            else:
                balance_loss = y_hat.new_tensor(0.0)

            total_loss = pre_loss + 0.1 * con_loss + balance_loss

            if getattr(self.args, "use_pmrl_svd", False):
                lambda_svd = getattr(self.args, "lambda_svd", 0.1)
                total_loss = total_loss + lambda_svd * svd_loss

            if getattr(self.args, "use_confu_loss", False):
                total_loss = total_loss + getattr(self.args, "lambda_confu", 0.1) * confu_loss
