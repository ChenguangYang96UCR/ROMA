...
class DreamKGGNNEncoder(nn.Module):
...
class MultiModalSelector(nn.Module):
    def __init__(self, input_dim, d_model=512, num_heads=1, num_layers=6, num_modal=5,args=None):  
        super().__init__()
        self.num_modal = args.data.num_modality
        ...
        self.dreamkg_encoder = DreamKGGNNEncoder(args.model.dreamkg_dim, input_dim, input_dim)
        self.dreamkg_embedding = nn.Parameter(torch.randn(input_dim))
    ...
    def forward(self, x, poi, satellite, dreamkg, loc, state, dreamkg_adj):
        B,N,L,D=x.shape
        poi_emb = self.poi_layer(poi)+self.poi_embedding  
        dreamkg_emb = self.dreamkg_encoder(dreamkg, dreamkg_adj) + self.dreamkg_embedding 
        ...
        modality_matrix = torch.stack(
            [poi_emb, satellite_emb, dreamkg_emb, loc_emb, x_cls_emb],
            dim=1
        )
        state_expanded = state.unsqueeze(-1)  
        emb = modality_matrix    
        if getattr(self.args, "disable_mm_selector", False):
            emb1 = self.linear(emb)
            x_emb = self.linear(x_emb)

            if getattr(self.args, "force_all_modalities", False):
                binary_mask = torch.ones_like(state_expanded)
            else:
                binary_mask = state_expanded.float()

            if (binary_mask.sum(dim=1) == 0).any():
                raise ValueError(
                    "No active modality after disabling selector. "
                    "Use state [1,1,1,1,1] for full no-selector, or [0,0,0,0,1] for temporal-only."
                )

            return emb1, None, None, x_emb, binary_mask, None, None
...
class CrossNode(nn.Module):
    def __init__(self, d_model, n_heads, attn_dropout=0.1, dropout=0.6, norm="LayerNorm", use_weights=True, k=20):
        super().__init__()
...
class mMOE(nn.Module):
    def __init__(self,d_model, n_heads, num_experts, modality_num=4, attn_dropout=0.05,top_k=2):
        ...
        self.modality_experts = nn.ModuleList([
            CrossNode(d_model, n_heads, attn_dropout=attn_dropout, dropout=0.3, norm="LayerNorm")
        ...
        self.linear2 = nn.ModuleList([nn.Linear(d_model, d_model) for _ in range(modality_num)])
...
class PMRLSVDLoss(nn.Module):
    def __init__(self, tau1=0.07, tau2=0.07):
        super().__init__()
        self.tau1 = tau1
        self.tau2 = tau2

    def forward(self, modalities):
        reps = [F.normalize(x, dim=-1) for x in modalities]
        stacked = torch.stack(reps, dim=-1)  # [B, D, M]
        u, s, _ = torch.linalg.svd(stacked, full_matrices=True)

        loss_intra = F.cross_entropy(
            s / self.tau1,
            torch.zeros(s.size(0), device=s.device, dtype=torch.long),
        )

        principal = u[:, :, 0]
        logits = principal @ principal.T / self.tau2
        labels = torch.arange(principal.size(0), device=principal.device)
