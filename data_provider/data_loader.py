...
class MultiDataModule:
    ...
    def collate(self, batch):
        ...
        dreamkg = self.datasets.city_dreamkg
        loc=self.datasets.city_loc
        adj=self.datasets.adj
        dreamkg_adj = self.datasets.dreamkg_adj
        state=self.datasets.city_state
        topk_indices=self.datasets.topk_indices
        topk_values=self.datasets.topk_values
        return x, start_days, start_intervals, poi, satellite, dreamkg, loc, adj, dreamkg_adj, y, mean, std, state, dataset, topk_indices, topk_values
...
class MM_data(Dataset):  
    def __init__(self, root_path: str,
                 stage:str="train",
                 dataset="gla",
                 data_avaliage={"poi":1, "map":1, "satellite":1, "location":1, "dreamkg": 1},
                 input_len: int = 6,
                 pred_len: int = 6,
                 state= [1,1,1,1],
                 interval: int = 1,
    ...
    def load_data(self): 
        ...
        # POI Load
        data=pd.read_csv(os.path.join(self.root_path,"poi",city+"_poi_vectors.csv"))  
        ...
        # DREAM-KG Load
        dreamkg_path = os.path.join(self.root_path, "data", city, "findhelp_zip_graph.npz")
        dreamkg_graph = np.load(dreamkg_path, allow_pickle=True)

        dreamkg_zipcodes = [str(z) for z in dreamkg_graph["zipcodes"].tolist()]
        if len(dreamkg_zipcodes) != len(indices):
            raise ValueError(f"DreamKG node count mismatch: {len(dreamkg_zipcodes)} vs {len(indices)}")

        self.city_dreamkg = torch.tensor(
            dreamkg_graph["findhelp_text_embeddings"],
            dtype=torch.float32
        )

        self.dreamkg_adj = torch.tensor(
            dreamkg_graph["adjacency"],
            dtype=torch.float32
        )
